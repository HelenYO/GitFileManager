...
main = do
  ...
  --initFileSystem "/Users/elena/Desktop/files"
...
loop
    ...
    --(convertString(foldMap pack d))
    --(convertString(toText d))
    --(convertString(toText d))
 ...
 = do
  ...
  case minput of
    ...
    Just args -> do
      if (args == [])
        then loop
        else case execParserPure (prefs idm) pinfo args
              --Success cmd -> liftIO (invoke cmd) >> loop
                   of
               Success io -> liftIO (io) >> loop
               Failure failure -> do
